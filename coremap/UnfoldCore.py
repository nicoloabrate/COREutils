...
# Author: N. Abrate
# File: UnfoldCore.py
# Description: This file contains a class to unfold a reactor core sector over
# a rotation angle provided by the user
...
class UnfoldCore:
    ...
    def __init__(self, geinp, rotangle):

        try:
            coremap = np.loadtxt(geinp, unpack=False, dtype=str, comments='%')
        except OSError:
            raise OSError("File %s not found" % geinp)

        # replace '*' with '0' for conversion
        coremap[coremap == '*'] = '0'
    ...
    def rot45(coremap, N):  # square geometry
        """
        This class defines a counterclockwise 45° rotation
        Parameters:
        =========== INPUT
            coremap: array[int]
    ...
    def rot60(coremap):  # hexagonal geometry
        """
        This class defines a counterclockwise 60° rotation
        Parameters:
        =========== INPUT
            coremap: array[int]
    ...
    def rot90(coremap, N):  # square geometry
        """
        This class defines a counterclockwise 90° rotation
        Parameters:
        =========== INPUT
            coremap: array[int]
    ...
    def rot180(coremap, N):  # square geometry
        """
        This class defines a counterclockwise 180° rotation
        Parameters:
        =========== INPUT
            coremap: array[int]
...
if __name__ == "__main__":
    ...
    try:
        ...
        startingmap[startingmap == '*'] = '0'
        startingmap = startingmap.astype(np.int)
    except OSError:
        print("%s not found!" % fname)
        raise OSError()

    core = UnfoldCore(fname, rotangle)
