...
class PostProcess:
    ...
    aliases = {'keff': 'eigenvalue dir.', 'k': 'eigenvalue dir.', 'flux': 'fluxdir',
               'adjoint': 'fluxadj', 'powerdens': 'powertot', 'rho': 'reactivity',
    ...
    def __init__(self, path):
        ...
        try:
            ...
            self.core = None
            print('WARNING: `core.h5` file is missing!')
        ...
        try:
            ...
            for i in range(self.core.nPre):
                self.integralParameters['intpar'].insert(idy, f'betaeff(i)')
    ...
    def get(self, which, hexa=None, time=None, z=None, pre=None,
        ...
        if which in self.distributions:
            ...
            NE = True
            if 'NEconfig' in self.core.__dict__.keys():
                isNE1D = False
            else:
                NE = False
            ...
            # TODO decide what to do with geometry data printed by FRENETIC
            nodes = self.core.NEAxialConfig.AxNodes
        ...
        with open(os.path.join(path, 'neutronic', 'outputNE.log'), 'r') as f:
            for l in f:
                if '@ D3DMATI: MAXVAL(SPECTRAL NORM)' in l:
                    ...
                    IK, IG = num.split()
                    IK, IG = int(IK), int(IG)
        nElz = len(self.core.NEAxialConfig.AxNodes)
        myIK = 0
        for iz in range(0, nElz):
            for ih in range(1, self.core.NAss+1):
                if myIK == IK:
                    hexty = self.core.getassemblytype(ih, isfren=True)
                    hexty = self.core.NEassemblytypes[hexty]
                    print(ih)
                    z = self.core.NEAxialConfig.AxNodes[iz]
...
def get(data, which, hexa, time=None, z=None, pre=None,
    ...
    if core is not None:
        ...
        if data.shape[Haxis] < self.core.NAss:
    ...
    nodes = self.core.NEAxialConfig.AxNodes
    if z is not None:
        if data.shape[Zaxis] < self.core.NEAxialConfig.AxNodes.shape[0]:
            nodes = myslice['nelz']
